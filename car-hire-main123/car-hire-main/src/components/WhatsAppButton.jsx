...
const WhatsAppButton = ({ phoneNumber, children, className, type = 'message' }) => {
  ...
  const handleClick = () => {
    if (type === 'call') {
      // For calls, try the whatsapp:// protocol first
      window.location.href = whatsappUrl;
      // Fallback to regular chat if call doesn't work after a short delay
      setTimeout(() => {
        window.open(formatWhatsAppLink(phoneNumber, 'message'), '_blank');
      }, 500);
    } else {
